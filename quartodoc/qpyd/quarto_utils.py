...
def qmd2py(qmd_path, py_path=None, keep_text=True):
    ...
    for i, line in enumerate(lines, start=1):
        ...
        if stripped.startswith("```{python}"):
            if in_block:
                raise ValueError(
                    f"Nested or unclosed code block: new block at line {i}, previous block started at line {block_start_line}"
                )
...
def execute_notebooks(*args, folders=None, tidy=True, debug=False):
    ...
    if args:
        ...
        for folder in folders:
            ...
            notebooks += [
                folder_path / f for f in sc.getfilepaths(folder_path, "*.qmd")
            ]
    ...
    def execute(i, path, pause=1.0):
        ...
        sc.timedsleep(
            delay
        )  # For some reason the interval argument to parallelize() isn't working
